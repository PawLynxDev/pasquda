...
export async function generateMetadata({
  ...
  const typeLabel =
    roast.roast_type === "linkedin"
      ? "a LinkedIn profile"
      : roast.roast_type === "resume"
        ? "a resume"
        : roast.domain;

  return {
    title: `Pasquda roasted ${typeLabel} — Score: ${roast.score}/100`,
    description: roast.summary,
    openGraph: {
      title: `Pasquda roasted ${typeLabel} — Score: ${roast.score}/100`,
      description: `"${roast.summary}" — Think yours is better?`,
      images: [`${appUrl}/api/og/${roast.id}`],
      url: `${appUrl}/roast/${roast.id}`,
    },
    twitter: {
      card: "summary_large_image",
      title: `Pasquda roasted ${typeLabel} — ${roast.grade} grade`,
      description: roast.summary,
      images: [`${appUrl}/api/og/${roast.id}`],
    },
  };
